...
- `<svg>` の直下だけでなく、`<g>` の中も一段ずつ降りて図形を拾う。デザインツール
  の書き出しは中身が `<g>` に入っているため。
- `transform` は、平行移動（`translate` の並び）であれば図形の座標に畳み込む。
  transform を持ったままだと、描画は合っていても選択ハンドルや当たり判定が
  元の x/y 側に残り、絵とモデルが食い違う。
- 畳み込めない `transform`（`rotate` / `scale` / `matrix`、および `d` を書き換え
  られない `<path>`）は文字列のまま保持し、描画にも書き出しにも同じものを出す。
  この場合ハンドルの位置は図形に追随しない。
- 一つも図形を取り込めなかった場合、シーンは触らず、`importSvg` は `false` を
  返す。読めなかったことを呼び出し側が知る手段がないと、成功と区別できない。
